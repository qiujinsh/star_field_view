...
if uploaded_file is not None:
    ...
    try:
        ...
        with sidebar_check_placeholder.container():
            ...
            df_north = pd.DataFrame({"点": range(1, 11), "赤经 RA (deg)": n_ra, "赤纬 Dec (deg)": n_dec})
            ...
            df_zenith = pd.DataFrame({"点": range(1, 11), "方位角 Az (deg)": z_altaz.az.deg, "高度角 Alt (deg)": z_altaz.alt.deg})
        ...
        ax2.text(x_center + dx_slit + 10, y_center + dy_slit, 'Slit (113°)', color='yellow', fontsize=14, fontweight='bold')
